<?php if (isset($_SESSION['alerta'])) : ?>
    <div onclick="this.style.display='none'" class="bg-green-100 cursor-pointer my-4 bg-opacity-70 border border-green-600 p-3 rounded-lg">
        <h2 class="font-medium mb-2">Foro Creado !</h2>
        <p class="text-gray-600">El foro ha sido creado exitosamente</p>
    </div>
...
<?php if (isset($_SESSION['editado'])) : ?>
    <div onclick="this.style.display='none'" class="bg-yellow-100 cursor-pointer my-4 bg-opacity-70 border border-yellow-600 p-3 rounded-lg">
        <h2 class="font-medium mb-2">Foro Editado !</h2>
        <p class="text-gray-600">El foro ha sido editado exitosamente</p>
    </div>
    ...
    <div class="border border-red-600 bg-red-50 mx-8 rounded-md p-4 flex flex-col gap-6">
        <div class="flex justify-between">
            <div class="flex items-center gap-2">
                ...
                <h2 class="font-bold">Eliminar Foro</h2>
            </div>
            <img onclick="toggleModal()" class="cursor-pointer" width="24" src="/imagenes/quit.svg" alt="">
        </div>
        <p class="text-gray-500">Si eliminas este foro no habra forma de recuperarlo</p>
...
<section class="overflow-auto">
    <table class="bg-gray-50 w-full rounded-md">
        <thead>
            <tr class="text-sm">
                <th class="p-4 text-start">Numero de foro</th>
                <th class="p-4 text-start">Fecha de creacion</th>
                <th class="p-4 text-start">Nombre del foro</th>
                <th class="p-4 text-start">Descripcion</th>
        ...
        <tbody>
            <?php foreach ($foros as $foro) : ?>
                <tr class=" odd:bg-[#00FF951A]">
                    <td class="p-4">#<?= $foro->getData('id') ?></td>
                    <td class="p-4"><?= $foro->getData('fecha_creacion') ?></td>
                    <td class="p-4"><?= $foro->getData('nombre') ?></td>
                    <td class="p-4 break-words max-w-md"><?= $foro->getData('descripcion') ?></td>
                    <td class="p-4"><img width="120" class=" min-w-[150px] aspect-auto object-cover rounded-lg" src="<?= $foro->getData('imagen_url') ?>" alt=""></td>
                    <td class="p-4">
                        <div class="grid grid-cols-2 gap-2">
                            <a href="/perfil/foro/editar?id=<?=$foro->id?>">
                                <img width="28" src="/imagenes/editTable.svg" alt="">
                            </a>
                            <img onclick="eliminarModal(this)" class="cursor-pointer" data-id="<?= $foro->getData('id') ?>" width="28" src="/imagenes/delete.svg" alt="">
                        </div>
                    </td>
                </tr>
            <?php endforeach ?>
